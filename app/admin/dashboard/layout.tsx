...
import { AdminDashboardContext } from "./dashboard-context"

const HEADER_COPY: Record<string, { title: string; description: string }> = {
  "/admin/dashboard/analytics": {
    title: "Dashboard Admin",
    description: "Monitoring & pengelolaan tiket divisi",
  },
  "/admin/dashboard/tickets": {
    title: "Kelola Tiket",
    description: "Kelola dan proses ticket dari user",
  },
  "/admin/dashboard/create-ticket": {
    title: "Buat Tiket Baru",
    description: "Buat tiket atas nama admin",
  },
}
...
function AdminDashboardShell({ children }: { children: React.ReactNode }) {
  ...
  const handleTicketClick = (ticketId?: number | string | null) => {
    ...
    router.push(`/admin/dashboard/tickets?ticketId=${id}`)
  }
  ...
  const headerCopy = HEADER_COPY[pathname] || HEADER_COPY["/admin/dashboard/analytics"]
  const headerTitle = pathname === "/admin/dashboard/analytics"
    ...
    <div className="flex h-screen bg-white dark:bg-black">
      ...
      <main className={cn(
        ...
        <div className="p-6 space-y-6 bg-white dark:bg-black">
          <AdminDashboardContext.Provider value={{ token, userDivision, refreshTrigger, bumpRefresh }}>
            {children}
